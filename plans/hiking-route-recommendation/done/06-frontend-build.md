...
- [x] 06.1 相關規格已審查 — **簽名**: 2026-09-18 14:18
- [x] 06.2 交付物已審查 — **簽名**: 2026-09-18 14:18
...
| # | 交付物 | 路徑 | 狀態 |
|---|--------|------|------|
| 06.1 | Frontend 骨架 | `C:\Users\retailingdata\Desktop\ruby\HikePilot\frontend\` | DONE |
| 06.2 | MSW Handlers | `C:\Users\retailingdata\Desktop\ruby\HikePilot\frontend\src\mocks\handlers\` | DONE |
| 06.3 | Pages | `C:\Users\retailingdata\Desktop\ruby\HikePilot\frontend\src\app\` | DONE |
...
### Chrome Test Guard 執行紀錄

實際瀏覽器操作完整跑過登入 → 問卷（含必填驗證）→ 推薦（含重新推薦）→ 選路線 → 規劃書，
全程無 console error。過程中發現並修復一個真實 bug：React StrictMode 下 `worker.start()`
被併發呼叫兩次，導致重新整理 `/plan` 頁時請求繞過 MSW 直接打到不存在的後端而 500。
修復見 `frontend/src/mocks/browser.ts`（singleton start promise + 等待 SW controller 就緒），
修復後重新驗證通過。
...
```bash
cd C:\Users\retailingdata\Desktop\ruby\HikePilot/frontend && npm run dev &
```

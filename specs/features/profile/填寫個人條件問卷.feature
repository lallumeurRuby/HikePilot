...
Feature: 填寫/更新個人條件問卷
  ...
  Rule: 前置（參數）- Q1、Q2、Q2B、Q3、Q4、Q5、Q11 為必答題，Q6-Q10 可選擇不回答
  ...
  Rule: 前置（參數）- 選項型題目的答案必須是題目定義的選項代碼
    Example: 提交不存在的選項代碼時操作失敗
      Given 系統中有以下使用者：
        | google_id | email              | name  |
        | g-001     | alice@example.com  | Alice |
      When 使用者 "Alice" 提交問卷回答：
        ...
        | Q5             | steepness:Q5-4,scrambling:Q5-1,exposure:Q5-2,altitude:Q5-3,navigation_difficulty:Q5-3 |
        | Q11            | Q11-1         |
      Then 操作失敗，violation_type 為 "INVALID_OPTION_CODE"
  ...
  Rule: 後置（狀態）- 使用者於每次請求路線推薦前，可選擇是否更新問卷答案
    Example: 再次提交覆蓋既有答案
      ...
      When 使用者 "Alice" 提交問卷回答：
        ...
        | Q1             | Q1-6         |
      Then 操作成功
      And 使用者 "Alice" 的問卷回答 "Q1" 應為 "Q1-6"
